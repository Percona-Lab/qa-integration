...
docker compose -f docker-compose-psmdb.yml build
docker compose -f docker-compose-psmdb.yml up -d

#Add users
docker compose -f docker-compose-psmdb.yml exec -T psmdb-server mongo --quiet << EOF
db.getSiblingDB("admin").createUser({ user: "root", pwd: "root", roles: [ "root", "userAdminAnyDatabase", "clusterAdmin" ] });
EOF
docker compose -f docker-compose-psmdb.yml exec -T psmdb-server mongo --quiet "mongodb://root:root@localhost/?replicaSet=rs0" < init/setup_psmdb.js

#Configure PBM
docker compose -f docker-compose-psmdb.yml exec -T psmdb-server bash -c "echo \"PBM_MONGODB_URI=mongodb://pbm:pbmpass@127.0.0.1:27017\" > /etc/sysconfig/pbm-agent"
docker compose -f docker-compose-psmdb.yml exec -T psmdb-server systemctl restart pbm-agent

#Configure PMM
set +e
i=1
while [ $i -le 3 ]; do
    output=$(docker compose -f docker-compose-psmdb.yml exec -T psmdb-server pmm-agent setup --server-insecure-tls 2>&1)
    exit_code=$?
...
docker compose -f docker-compose-psmdb.yml exec -T psmdb-server pmm-admin add mongodb psmdb-server_${random_number} --username=pmm_mongodb --password="5M](Q%q/U+YQ<^m" --host psmdb-server --port 27017 --tls --tls-certificate-key-file=/mongodb_certs/client.pem --tls-ca-file=/mongodb_certs/ca-certs.pem --cluster=mycluster

#Add some data
docker compose -f docker-compose-psmdb.yml exec -T psmdb-server mgodatagen -f /etc/datagen/replicaset.json --username=pmm_mongodb --password="5M](Q%q/U+YQ<^m" --host psmdb-server --port 27017 --tlsCertificateKeyFile=/mongodb_certs/client.pem --tlsCAFile=/mongodb_certs/ca-certs.pem

tests=${TESTS:-yes}
if [ $tests = "yes" ]; then
    echo "running tests"
    docker compose -f docker-compose-psmdb.yml run test pytest -s -x --verbose test.py
    else
    echo "skipping tests"
fi
