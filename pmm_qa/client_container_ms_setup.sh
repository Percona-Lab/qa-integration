...
if [[ "$number_of_nodes" == 1 ]];then
   if [[ ! -z $group_replication ]]; then
      dbdeployer deploy --topology=group replication ${db_version_sandbox} --single-primary --sandbox-binary=~/ms${ms_version} --remote-access=% --bind-address=0.0.0.0 --force ${my_cnf_options:+--my-cnf-options="$my_cnf_options"} --my-cnf-options="performance_schema_max_sql_text_length=262144"
      ...
      dbdeployer deploy single ${db_version_sandbox} --sandbox-binary=~/ms${ms_version} --port=$MS_PORT --remote-access=% --bind-address=0.0.0.0 --force ${my_cnf_options:+--my-cnf-options="$my_cnf_options"} --my-cnf-options="performance_schema_max_sql_text_length=262144"
      export db_sandbox=$(dbdeployer sandboxes | awk -F' ' '{print $1}')
   ...
   if [[ ! -z $group_replication ]]; then
     ...
     dbdeployer deploy multiple ${db_version_sandbox} --sandbox-binary=~/ms${ms_version} --nodes $number_of_nodes --force --remote-access=% --bind-address=0.0.0.0 ${my_cnf_options:+--my-cnf-options="$my_cnf_options"} --my-cnf-options="performance_schema_max_sql_text_length=262144"
     export db_sandbox=$(dbdeployer sandboxes | awk -F' ' '{print $1}')
