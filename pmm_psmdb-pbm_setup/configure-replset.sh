...
docker-compose -f docker-compose-rs.yaml exec -T rs101 mongo --quiet --eval << EOF
    config = {
        "_id" : "rs",
        "members" : [
        {
...
docker-compose -f docker-compose-rs.yaml exec -T rs101 mongo "mongodb://root:root@localhost/?replicaSet=rs" --quiet --eval << EOF
